  ...
  const columns: Column<Manager>[] = [
    {
      ...
      cell: (row) => (
        <div className="flex items-center gap-3">
          <div className="p-2.5 rounded-xl bg-gray-100 border border-gray-600 text-gray-700">
            <UserCheck className="w-5 h-5" />
          </div>
          <div>
            <span className="font-bold text-gray-900 text-sm block">{row.fullName}</span>
        ...
        <div className="flex flex-wrap gap-1 max-w-xs">
          {row.assignedWarehouses && row.assignedWarehouses.length > 0 ? (
            row.assignedWarehouses.map((w) => (
              <span
                key={w.warehouseId}
                className="px-2 py-0.5 rounded-md bg-[#F5F5F5] border border-[#E5E5E5] text-[#666666] text-[11px] font-medium flex items-center gap-1"
              >
                <WarehouseIcon className="w-3 h-3 text-[#666666]" />
                {w.warehouseName}
              </span>
            ))
          ) : (
            <span className="text-xs text-gray-400 italic">No Warehouses</span>
          )}
        </div>
      ),
    },
    ...
    {
      header: 'Created Date',
      cell: (row) => <span className="text-xs text-gray-500 font-mono">{row.createdAt}</span>,
    },
    {
      header: "Actions",
      cell: (row) => (
        <div className="flex items-center gap-1.5">
          ...
          >
            <KeyRound className="w-4 h-4 text-gray-500 hover:text-[#666666]" />
          </Button>
          <Button
            size="sm"
            variant="ghost"
    ...
    <div className="flex flex-col gap-6 text-left max-w-7xl mx-auto pb-12">
      ...
      <Card title={`Operational Managers (${filteredManagers.length})`}>
        {loading ? (
          ...
          <DataTable columns={columns} data={paginatedManagers} keyExtractor={(item) => item.id.toString()} pagination={{ currentPage, totalItems: filteredManagers.length, pageSize, onPageChange: setCurrentPage }} />
        )}
      </Card>
        ...
        >
          <form onSubmit={handleResetPasswordSubmit} className="flex flex-col gap-4 text-left">
            <p className="text-xs text-gray-500">
              Enter a new secure password for <strong className="text-gray-800">@{resetModalManager.username}</strong>.
            </p>
            ...
            <div>
              ...
              <input
                ...
                className="w-full bg-[#F7F8FA] border border-gray-200 rounded-xl px-3 py-2 text-sm text-gray-900 focus:outline-none focus:border-[#D4D4D4]"
              />
            </div>
